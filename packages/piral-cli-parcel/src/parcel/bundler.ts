...
import {
  ...
  combineExternals,
} from "parcel-plugin-externals/utils";
...
function gatherExternals(
  bundle: Bundler.ParcelBundle,
  gatheredExternals: Array<string> = []
) {
...
async function applySourceMapShift(
  sourceFile: string,
  lineOffset = 1
): Promise<string> {
...
function getRequireId(importmap: Array<SharedDependency>, depName: string) {
  const dependency = importmap.find((i) => i.name === depName);
  return (dependency as any)?.requireId ?? dependency?.id ?? depName;
}

/**
 * Transforms a pilet's bundle to a SystemJS entry module.
 */
async function wrapSystemJs(
  details: WrapDetails,
  content: string
): Promise<string> {
  const { minified, map, prName, head, importmap, externals } = details;
  const dependencies = externals.map((depName) =>
    getRequireId(importmap, depName)
  );
  const originalRequire = minified
    ? '"function"==typeof parcelRequire&&parcelRequire'
    : "typeof parcelRequire === 'function' && parcelRequire";
